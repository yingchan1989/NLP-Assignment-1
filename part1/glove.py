...
def wordids_to_tensors(wordids, embedding_dim, vocab_size, seed=0):
    ...
    m_ = tf.Variable(tf.random_uniform([V, E], -1.0, 1.0), name="m")

    B_ = tf.Variable(tf.zeros([V, ], dtype=tf.float32), name="B")

    b_ = tf.nn.embedding_lookup(B_, wordids)

    x_ = tf.nn.embedding_lookup(m_, wordids)
...
def example_weight(Xij, x_max, alpha):
    ...
    '''
    # START YOUR CODE
    fx = tf.pow(tf.div(tf.minimum(Xij, x_max), x_max), alpha)
    return fx
    # END YOUR CODE
...
def loss(w, b, w_c, b_c, c):
    ...
    '''
    # START YOUR CODE
    fx = example_weight(c, 100, 0.75)
    loss_part = (tf.diag_part(tf.matmul(w, tf.transpose(w_c))) + b + b_c - tf.log(c))
    loss = tf.mul(fx, tf.pow(loss_part,2))
    return loss
