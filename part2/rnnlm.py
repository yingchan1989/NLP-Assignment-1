...
class RNNLM(object):
  ...
  def BuildCoreGraph(self):
    ...
    lstm_cell = MakeFancyRNNCell(H=self.H, keep_prob=self.dropout_keep_prob_, num_layers=self.num_layers)
    self.initial_h_ = tf.rnn.cell.zero_state
    outputs, states = tf.rnn.rnn(lstm_cell, self.input_w_, initial_state=self.initial_h)
    ...
    self.loss_ = tf.nn.sparse_softmax_cross_entropy_with_logits(pred, y)
  ...
  def BuildTrainGraph(self):
    ...
    with tf.name_scope("Train_Loss"):
      ...
      self.train_loss_ = tf.reduce_sum(self.loss_)


    # Define optimizer and training op
    with tf.name_scope("Training"):
      self.train_step_ = tf.train.tf.train.AdagradOptimizer(learning_rate=self.learning_rate_).minimize(self.train_loss)
      # Placeholder: replace with an actual op
